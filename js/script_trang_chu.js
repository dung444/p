...
function initHeaderUserState() {
  ...
  if (currentUser) {
    ...
    loginLink.style.visibility = "hidden"; // để giữ layout nhưng ẩn
    logoutBtn.style.display = "block";
    logoutBtn.innerHTML =
      '<span><i class="fa-solid fa-user"></i> Thông tin cá nhân</span>';
    logoutBtn.onclick = () => {
...
function initBrandFilterSection() {
  ...
  if (!brandButtons.length || !productsContainer) return;
  ...
  brandButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // Ưu tiên product.brand, fallback sang name/description chứa brand
      ...
      renderBrandProducts(filtered, brand);
    });
  });
}
